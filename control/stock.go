...
var (
	...
	OrderLimitMap        = map[string]string{"系统得分": "score", "收盘涨幅": "percent", "收盘价格": "price", "基金数量": "fund_count", "私募数量": "sm_count", "分红次数": "fenghong_count", "送股次数": "songgu_count", "转增次数": "zhuangzeng_count", "配股次数": "peigu_count", "增发次数": "zengfa_count", "控股公司数量": "subcomp_count"}
	OrderTypeMap         = map[string]string{"升序": "asc", "降序": "desc"}
	//OrderLimit           = []string{"score", "percent", "price", "fund_count", "sm_count", "fenghong_count", "songgu_count", "zhuangzeng_count", "peigu_count", "zengfa_count", "subcomp_count"}
...
func (d *PredictControl) GetWeekDetail(c *gin.Context) {
	...
	case "week":
		...
		store.MysqlClient.GetDB().Model(&dal.TicketHistoryWeekly{}).Where("code = ? and date <= ?", code, date).Limit(80).Order("date desc").Find(&TicketHistoryTmp)
	...
	case "month":
		...
		store.MysqlClient.GetDB().Model(&dal.TicketHistoryMonth{}).Where("code = ? and date <= ?", code, date).Limit(80).Order("date desc").Find(&TicketHistoryTmp)
		for i := len(TicketHistoryTmp) - 1; i >= 0; i-- {
			TicketHistory = append(TicketHistory, TicketHistoryTmp[i])
		}
		d.Response(c, TicketHistory, nil)
...
func (d *PredictControl) GetDetail(c *gin.Context) {
	...
	store.MysqlClient.GetDB().Model(&dal.TicketHistory{}).Where("code = ? and date <= ?", code, date).Limit(80).Order("date desc").Find(&TicketHistoryTmp)
	for i := len(TicketHistoryTmp) - 1; i >= 0; i-- {
		TicketHistory = append(TicketHistory, TicketHistoryTmp[i])
	}
...
func (d *PredictControl) GetQueryList(c *gin.Context) {
	response := map[string][]string{"concept": ConceptList, "label": LabelList,
		"belong": BelongList, "location": LocationList, "forms": FormList, "tape": TapeList}
	d.Response(c, response, nil)
}
