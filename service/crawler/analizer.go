...
func (craw *Crawler) HasTopLineWeek(result *model.CalcResult, recent int) bool {
	closes := result.RecentCloseWeek[0:recent]
	for index, i := range closes {
		// 开盘价大于收盘价
		if result.RecentOpenWeek[index]-i >= 0 {
			virtual := result.RecentHighWeek[index] - result.RecentOpenWeek[index]
			if virtual > result.RecentCloseWeek[index+1]*0.04*3.5 {
				return true
			}
		} else {
			virtual := result.RecentHighWeek[index] - result.RecentCloseWeek[index]
			if virtual > result.RecentCloseWeek[index+1]*0.04*3.5 {
				return true
			}
		}
...
// 下影线是实体柱子4倍以上
func (craw *Crawler) HasLowLineWeek(result *model.CalcResult, recent int) bool {
	closes := result.RecentCloseWeek[0:recent]
	for index, i := range closes {
		// 开盘价大于收盘价
		if result.RecentOpenWeek[index]-i >= 0 {
			virtual := result.RecentCloseWeek[index] - result.RecentLowWeek[index]
			if virtual > result.RecentCloseWeek[index+1]*0.04 {
				return true
			}
		} else {
			virtual := result.RecentOpenWeek[index] - result.RecentLowWeek[index]
			if virtual > result.RecentCloseWeek[index+1]*0.04 {
				return true
			}
		}
...
func (craw *Crawler) Analyze(result *model.CalcResult, code, name string) {
	...
	// 周线长上影
	changshangying_week := craw.HasTopLineWeek(result, 1)
	changxiaying_week := craw.HasLowLineWeek(result, 1)
	...
	// 历史更名次数
	_, has_st := GetHistoryNameByCode(code)

	// 最近3天换手率大于 3%
	//to1 := result.RecentDailyData.RecentTurnoverRate[0] > 5
	...
	var wpriceaboveave1, wpriceaboveave2, wpriceaboveave3, wpriceaboveave4, wliangshangyang1, wliangshangyang2, wliangnengbuduanbigger, wliangnengtupo1, wliangnengtupo2 bool
	...
	var ypriceaboveave1, ypriceaboveave2, ypriceaboveave3, ypriceaboveave4, yliangshangyang1, yliangshangyang2, yliangnengbuduanbigger, yliangnengtupo1, yliangnengtupo2 bool
	...
	current_low, current_high := result.RecentLow[0], result.RecentHigh[0]
	// 3月新低、新高
	// 6月新低
	// 9月新低
	// 12月新低
	...
	// 周线新低
	if len(result.RecentLow) >= 5 {
		if JudgeNumber(current_low, result.RecentLow[0:5], "low") {
			bad_cond_str += "五日新低; "
		}

		if JudgeNumber(current_high, result.RecentLow[0:5], "high") {
			cond_str += "五日新高; "
		}
	}
	if len(result.RecentLow) >= 10 {
		if JudgeNumber(current_low, result.RecentLow[0:10], "low") {
			bad_cond_str += "十日新低; "
		}
		if JudgeNumber(current_high, result.RecentLow[0:6], "high") {
			cond_str += "十日新高; "
		}
	}
	...
	if changshangying_week {
		cond_str += "周线长上影; "
	}

	if changxiaying_week {
		cond_str += "周线长下影; "
	}
	...
	if jichuang1 {
		score -= 2
		bad_cond_str += "击穿5日支撑位; "
	}
	if jichuang2 {
		score -= 2
		bad_cond_str += "击穿10日支撑位; "
	}
	if jichuang3 {
		score -= 2
		bad_cond_str += "击穿30日支撑位; "
	}
	if jichuang4 {
		score -= 2
		bad_cond_str += "击穿60日支撑位; "
	}

	if jichuang5 {
		score -= 2
		bad_cond_str += "击穿5周支撑位; "
	}
	if jichuang6 {
		score -= 2
		bad_cond_str += "击穿10周支撑位; "
	}
	if jichuang7 {
		score -= 2
		bad_cond_str += "击穿30周支撑位; "
	}
	if jichuang8 {
		score -= 2
		bad_cond_str += "击穿60周支撑位; "
	}
	if jichuang9 {
		score -= 2
		bad_cond_str += "击穿5月支撑位; "
	}
	if jichuang10 {
		score -= 2
		bad_cond_str += "击穿10月支撑位; "
	}
	if jichuang11 {
		score -= 2
		bad_cond_str += "击穿30月支撑位; "
	}
	if jichuang12 {
		score -= 2
		bad_cond_str += "击穿60月支撑位; "
	}
	...
	if pricexiajiang2 {
		score -= 1
		bad_cond_str += "10日均线下降; "
	}
	if pricexiajiang3 {
		score -= 1
		bad_cond_str += "30日均线下降; "
	}
	if pricexiajiang4 {
		score -= 1
		bad_cond_str += "60日均线下降; "
	}
	...
	if wpricexiajiang2 {
		score -= 1
		bad_cond_str += "10周均线下降; "
	}
	if wpricexiajiang3 {
		score -= 1
		bad_cond_str += "30周均线下降; "
	}
	if wpricexiajiang4 {
		score -= 1
		bad_cond_str += "60周均线下降; "
	}
	...
	if ypricexiajiang2 {
		score -= 1
		bad_cond_str += "10月均线下降; "
	}
	if ypricexiajiang3 {
		score -= 1
		bad_cond_str += "30月均线下降; "
	}
	if ypricexiajiang4 {
		score -= 1
		bad_cond_str += "60月均线下降; "
	}
