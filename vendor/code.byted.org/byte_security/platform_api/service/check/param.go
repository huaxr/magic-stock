...
func (p *param) ParseSearchLikeWhere(c *gin.Context, searchFields ...string) (where string, args []interface{}) {
	search := c.DefaultQuery("search", "")
	if len(search) == 0 {
		return
	}
	var whereStr []string
	likeStr := p.addPrefix(search, "%")
	for _, field := range searchFields {
		whereStr = append(whereStr, "`"+field+"` like ?")
		args = append(args, likeStr)
	}
	where = strings.Join(whereStr, JORN_OR)
	where = " ( " + where + " ) "
	return
}

func (p *param) ParseSearchEqualWhere(c *gin.Context, searchFields ...string) (where string, args []interface{}) {
	search := c.DefaultQuery("search", "")
	if len(search) == 0 {
		return
	}
	var whereStr []string
	for _, field := range searchFields {
		whereStr = append(whereStr, "`"+field+"` = ?")
