...
func (s JoinTableHandler) getSearchMap(db *DB, sources ...interface{}) map[string]interface{} {
	...
	for _, source := range sources {
		scope := db.NewScopeWithKind(source, ScopeUnkown)
...
func (s JoinTableHandler) Add(handler JoinTableHandlerInterface, db *DB, source interface{}, destination interface{}) error {
	scope := db.NewScopeWithKind("", ScopeUnkown)
...
func (s JoinTableHandler) Delete(handler JoinTableHandlerInterface, db *DB, sources ...interface{}) error {
	var (
		scope      = db.NewScopeWithKind(nil, ScopeWrite)
		conditions []string
		values     []interface{}
	)
...
func (s JoinTableHandler) JoinWith(handler JoinTableHandlerInterface, db *DB, source interface{}) *DB {
	...
	if s.Source.ModelType == scope.GetModelStruct().ModelType {
		destinationTableName := db.NewScopeWithKind(reflect.New(s.Destination.ModelType).Interface(), ScopeUnkown).QuotedTableName()
		for _, foreignKey := range s.Destination.ForeignKeys {
