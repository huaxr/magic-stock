...
func (scope *Scope) ShouldRejectTestRequest() bool {
	if !scope.db.IsTestRequest() {
		return false
	}

	if switchVal, ok := scope.db.Ctx.Value(ContextStressSwitch).(string); ok {
		return switchVal == SwitchOff
	}
	return false
}
func (scope *Scope) SetScopeKind(kind string) {
	scope.kind = kind
}
...
func (scope *Scope) New(value interface{}) *Scope {
	return scope.NewWithKind(value, scope.kind)
}

func (scope *Scope) NewWithKind(value interface{}, scopeKind string) *Scope {
	return &Scope{db: scope.NewDB(), Search: &search{}, Value: value, kind: scopeKind}
}
...
func (scope *Scope) originTableName() string {
...
func (scope *Scope) TableName() string {
	originName := scope.originTableName()

	if scope.db.IsTestRequest() {
		if scope.kind == ScopeRead && scope.db.ShouldSkipTest() {
			return originName
		}
		return getPostfixedTableName(originName)
	}
	return originName
}
...
func (scope *Scope) Exec() *Scope {
	...
	if !scope.HasError() && !scope.ShouldRejectTestRequest() {
		if ctxer, ok := scope.SQLDB().(sqlCtxExecer); ok {
			if result, err := ctxer.ExecContext(scope.db.Ctx, scope.SQL, scope.SQLVars...); scope.Err(err) == nil {
				if count, err := result.RowsAffected(); scope.Err(err) == nil {
					scope.db.RowsAffected = count
				}
			}
		} else if result, err := scope.SQLDB().Exec(scope.SQL, scope.SQLVars...); scope.Err(err) == nil {
			if count, err := result.RowsAffected(); scope.Err(err) == nil {
				scope.db.RowsAffected = count
			}
		}
...
func (scope *Scope) row() *sql.Row {
	...
	if ctxer, ok := scope.SQLDB().(sqlCtxQuerier); ok {
		return ctxer.QueryRowContext(scope.db.Ctx, scope.SQL, scope.SQLVars...)
	}
...
func (scope *Scope) rows() (*sql.Rows, error) {
	...
	if ctxer, ok := scope.SQLDB().(sqlCtxQuerier); ok {
		return ctxer.QueryContext(scope.db.Ctx, scope.SQL, scope.SQLVars...)
	}
...
func (scope *Scope) related(value interface{}, scopeKind string, foreignKeys ...string) *Scope {
	toScope := scope.db.NewScopeWithKind(value, scopeKind)
