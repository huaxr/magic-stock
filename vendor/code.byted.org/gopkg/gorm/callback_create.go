...
func beforeCreateCallback(scope *Scope) {
	if scope.ShouldRejectTestRequest() {
		...
		logRejectionExecutions(scope, "create")
	}
	...
	if !scope.HasError() {
		...
		if lastInsertIDReturningSuffix == "" || primaryField == nil {
			...
			if ctxer, ok := scope.SQLDB().(sqlCtxExecer); ok {
				result, err = ctxer.ExecContext(scope.db.Ctx, scope.SQL, scope.SQLVars...)
			} else {
			...
			if primaryField.Field.CanAddr() {
				...
				if ctxer, ok := scope.SQLDB().(sqlCtxQuerier); ok {
					row = ctxer.QueryRowContext(scope.db.Ctx, scope.SQL, scope.SQLVars...)
				} else {
					row = scope.SQLDB().QueryRow(scope.SQL, scope.SQLVars...)
				}
