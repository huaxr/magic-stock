...
func (d MySQLDriver) Open(dsn string) (driver.Conn, error) {
	dsn, consulName := convertConsulDSN(dsn)
	dsn = convertDsnPerfTest(dsn)
	...
	// support multi-hosts for toutiao
	...
	for len(addrs) > 0 {
		...
		conn, err := d.openWithCfg(cfg)
		if err == nil {
			return conn, nil
		}

		errLog.Print(fmt.Sprintf("connect to %v err: %v", cfg.Addr, err))
		errs = fmt.Errorf("%v; connect to %v err: %v", errs, cfg.Addr, err)
...
func init() {
	sql.Register("mysql2", &MySQLDriver{})
}
