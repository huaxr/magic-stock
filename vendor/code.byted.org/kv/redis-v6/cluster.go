...
import (
	...
	"code.byted.org/kv/redis-v6/pkg/hashtag"
...
type ClusterOptions struct {
	...
	//
	OpInfo bool
	// set opapply for every connection
	OpApply bool
...
func (c *ClusterClient) cmdInfo(name string) *CommandInfo {
	...
	if info == nil {
		pkg.Logf("info for cmd=%s not found", name)
	}
	return info
}
...
func (c *ClusterClient) Watch(fn func(*Tx) error, keys ...string) error {
	...
	for attempt := 0; attempt <= c.opt.MaxRedirects; attempt++ {
		...
		moved, ask, addr := pkg.IsMovedError(err)
		if moved || ask {
			c.lazyReloadState()
			node, err = c.nodes.GetOrCreate(addr)
			if err != nil {
...
func (c *ClusterClient) WriteAofCommand(key string, cmd []byte) error {
	state, err := c.state()
	if err != nil {
		return err
	}
	...
	for attempt := 0; attempt <= c.opt.MaxRedirects; attempt++ {
		...
		if ask {
			buffer := proto.NewWriteBuffer()
			askCmd := NewCmd("ASKING")
			buffer.Append(askCmd.Args())
			buffer.AppendRawBytes(cmd)
			err = node.Client.WriteRawCmd(buffer.Bytes())
			ask = false
		} else {
			err = node.Client.WriteRawCmd(cmd)
		}
		...
		if c.opt.ReadOnly && pkg.IsLoadingError(err) {
			// TODO: race
...
func (c *ClusterClient) Process(cmd Cmder) error {
	...
	for attempt := 0; attempt <= c.opt.MaxRedirects; attempt++ {
		...
		if ask {
			...
			if c.opt.OpInfo {
				if c.opinfo == nil {
					panic("c.opinfo is nil")
				}
				pipe.Process(c.opinfo)
			}
...
func (c *ClusterClient) reaper(idleCheckFrequency time.Duration) {
	...
	for range ticker.C {
		...
		for _, node := range nodes {
			...
			if err != nil {
				pkg.Logf("ReapStaleConns failed: %s", err)
			} else {
				n += nn
			}
		}
...
func (c *ClusterClient) pipelineExec(cmds []Cmder) error {
	...
	for attempt := 0; attempt <= c.opt.MaxRedirects; attempt++ {
		...
		for node, cmds := range cmdsMap {
			...
			if err == nil || pkg.IsRedisError(err) {
...
func (c *ClusterClient) mapCmdsByNode(cmds []Cmder) (map[*clusterNode][]Cmder, error) {
	...
	for _, cmd := range cmds {
		...
		if c.opt.OpInfo {
			...
			cmdsMap[node] = append(cmdsMap[node], c.opinfo)
		}
...
func (c *ClusterClient) txPipelineReadQueued(
	...
	for _, cmd := range cmds {
		...
		if c.checkMovedErr(cmd, err, failedCmds) || pkg.IsRedisError(err) {
			continue
		}
