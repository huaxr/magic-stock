...
func (d *sentinelFailover) masterAddr() (string, error) {
	...
	if d.sentinel != nil {
		...
		if err == nil {
			...
			pkg.Logf("sentinel: master=%q addr=%q", d.masterName, addr)
			return addr, nil
		}

		pkg.Logf("sentinel: GetMasterAddrByName name=%q failed: %s", d.masterName, err)
		d._resetSentinel()
	}
	...
	for i, sentinelAddr := range d.sentinelAddrs {
		...
		if err != nil {
			pkg.Logf("sentinel: GetMasterAddrByName master=%q failed: %s", d.masterName, err)
			sentinel.Close()
			continue
		}
...
func (d *sentinelFailover) discoverSentinels(sentinel *sentinelClient) {
	...
	if err != nil {
		pkg.Logf("sentinel: Sentinels master=%q failed: %s", d.masterName, err)
		return
	}
	...
	for {
		if pubsub == nil {
			...
			if err := pubsub.Subscribe("+switch-master"); err != nil {
				pkg.Logf("sentinel: Subscribe failed: %s", err)
				pubsub.Close()
				d.resetSentinel()
				return
			}
		...
		if err != nil {
			pkg.Logf("sentinel: ReceiveMessage failed: %s", err)
			pubsub.Close()
			d.resetSentinel()
			return
		}
		...
		case "+switch-master":
			...
			if parts[0] != d.masterName {
				pkg.Logf("sentinel: ignore addr for master=%q", parts[0])
				continue
			}
