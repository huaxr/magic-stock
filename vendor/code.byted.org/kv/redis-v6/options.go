...
type Options struct {
	...
	// Amount of time after which client closes exist connections.
	// Default is 1 hour
	...
	// Toutiao gdpr redis token
	GetRedisDpsToken    func(bool) string
	VerifyRedisDpsReply func(string) error
}
...
func (opt *Options) init() {
	...
	if opt.LiveTimeout > 0 && opt.LiveTimeout < time.Hour {
		opt.LiveTimeout = time.Hour
	}
