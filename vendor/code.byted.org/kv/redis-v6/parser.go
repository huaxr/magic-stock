...
// Implements proto.MultiBulkParse
func byteSliceParser(rd *proto.Reader, n int64) (interface{}, error) {
	bs := make([][]byte, 0, n)
	for i := int64(0); i < n; i++ {
		b, err := rd.ReadBytesReply()
		if err == Nil {
			bs = append(bs, nil)
		} else if err != nil {
			return nil, err
...
// Implements parse the ttlQItem

func ttlQItemParser(rd *proto.Reader, n int64) (interface{}, error) {
	ttlQItems := make([]TtlQItem, n/2)
	for i := int64(0); i < n; i += 2 {
		var err error

		item := &ttlQItems[i/2]

		item.cursor, err = rd.ReadIntReply()
		if err != nil {
			return nil, err
		}

		item.item, err = rd.ReadStringReply()
		if err != nil {
			return nil, err
		}
	}
	return ttlQItems, nil
}
