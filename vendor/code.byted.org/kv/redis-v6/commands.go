...
func (c *cmdable) del(cmd_name string, keys ...string) *IntCmd {
...
func (c *cmdable) CGet(key string) *IntCmd {
	cmd := NewIntCmd("cget", key)
	c.process(cmd)
	return cmd
}

func (c *cmdable) DpsAddKey(key string) *StatusCmd {
	cmd := NewStatusCmd("dpsaddkey", key)
	c.process(cmd)
	return cmd
}
...
func (c *cmdable) CIncr(key string) *IntCmd {
	cmd := NewIntCmd("cincr", key)
	c.process(cmd)
	return cmd
}
...
func (c *cmdable) CIncrBy(key string, value int64) *IntCmd {
	cmd := NewIntCmd("cincrby", key, value)
	c.process(cmd)
	return cmd
}
...
func (c *cmdable) CMGet(keys ...string) *SliceCmd {
	args := make([]interface{}, 1+len(keys))
	args[0] = "cmget"
	for i, key := range keys {
		args[1+i] = key
...
func (c *cmdable) set(cmd_name string, key string, value interface{}, expiration time.Duration) *StatusCmd {
...
func (c *cmdable) Set(key string, value interface{}, expiration time.Duration) *StatusCmd {
	return c.set("set", key, value, expiration)
}

// toutiao international counter service
func (c *cmdable) CSet(key string, value int64, expiration time.Duration) *StatusCmd {
	return c.set("cset", key, value, expiration)
}
...
func (c *cmdable) shutdown(modifier string) *StatusCmd {
	...
	if err := cmd.Err(); err != nil {
		...
		cmd.err = pkg.RedisError(cmd.val)
		cmd.val = ""
	}
	return cmd
}
...
func (c *statefulCmdable) OpApply() *StatusCmd {
	cmd := NewStatusCmd("opapply")
	c.process(cmd)
	return cmd
}
...
func (c *cmdable) XGet(key string) *XGetCmd {
	cmd := NewXGetCmd("xget", key)
	c.process(cmd)
	return cmd
}

func (c *cmdable) XSet(key string, value interface{}, generation int64, expiration time.Duration) *XSetCmd {
	args := make([]interface{}, 4, 6)
	args[0] = "xset"
	args[1] = key
	args[2] = value
	args[3] = generation
...
func (c *cmdable) ScanRow(key string, limit int, offset int, target string) *ScanRowCmd {
	cmd := NewScanRowCmd("scanrow", key, limit, offset, target)
	c.process(cmd)
	return cmd
}
...
type TtlQItem struct {
	cursor int64
	item   string
}

func (c *cmdable) TtlQScan(key string, startCursor, endCursor, limit int64) *TtlQScanCmd {
	cmd := NewTtlQScanCmd("ttlqscan", key, startCursor, endCursor, limit)
	c.process(cmd)
	return cmd
}
