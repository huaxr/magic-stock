...
func (c *baseClient) releaseConn(cn *pool.Conn, err error) bool {
	if c.releaseConnAddition != nil {
		return c.releaseConnAddition(cn, err)
	} else {
...
func (c *baseClient) InitConn(cn *pool.Conn) error {
	if c.opt.Password == "" &&
		...
		c.opt.OnConnect == nil &&
		!c.opt.opApply &&
		c.opt.GetRedisDpsToken == nil &&
		c.opt.VerifyRedisDpsReply == nil {
		cn.Inited = true
		return nil
	}
	if cn.Inited {
	...
	if c.opt.GetRedisDpsToken != nil && c.opt.VerifyRedisDpsReply != nil {
		token := c.opt.GetRedisDpsToken(false)
		cmd := NewStringCmd("dpsauth", token)
		...
		err = c.opt.VerifyRedisDpsReply(resp)
		if err != nil {
			return err
		}
	}
...
func (c *baseClient) WrapGetConn(fn func() (*pool.Conn, bool, error)) {
	c.getConnAddition = fn
}

func (c *baseClient) WrapReleaseConn(fn func(*pool.Conn, error) bool) {
...
func (c *baseClient) WriteRawCmd(b []byte) error {
	lastErr := errors.New("unexpert error.")
	for attempt := 0; attempt <= c.opt.MaxRetries; attempt++ {
		...
		_, err = cn.Rd.ReadReply(sliceParser)
		c.releaseConn(cn, err)
		if err == Nil {
			return nil
		}
